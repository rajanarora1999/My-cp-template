...
*/
vector <vvi> maps(503, vvi(503, vi(22)));
void solve(){
int n,m,k;
cin>>n>>m>>k;
vvi rowex(n,vi (m-1)),colex(n-1,vi (m));
f(i,0,n,1)
f(j,0,m-1,1)
cin>>rowex[i][j];
f(i,0,n-1,1)
...
for(int k=1;k<=10;k++)
{
        ...
        for(int j=0;j<m;j++)
    {
        ...
        if(i>0)
        a=maps[i-1][j][k-1]+colex[i-1][j];
        if(i+1<n)
        b=maps[i+1][j][k-1]+colex[i][j];
        if(j>0)
        c=maps[i][j-1][k-1]+rowex[i][j-1];
        if(j+1<m)
        d=maps[i][j+1][k-1]+rowex[i][j];
        maps[i][j][k]=min(min(a,b),min(c,d));
    }
}
